...
exports.prototype = {
    ...
    _deliver_transmit_results: function (_results, _callback) {
      ...
      async.each(_results,
        ...
        function (_err) {
          ...
          if (self._is_queue_disabled) {

            /* Non-queueing mode:
                Throw the entire outbound queue away. Our instansiator
                has a copy of any failed messages, and will resubmit them. */
    ...
    initialize: function (_options) {
      ...
      self._is_debug_enabled = !!self._options.debug;

      self._is_queue_disabled = (
        _.isBoolean(self.options.queue) && !self.options.queue
      );
